...
impl StorageManagement for Contract {
    ...
    fn storage_deposit(
        ...
        if deposited == 0 {
            ...
            assert!(
                Contract::suggested_min_storage_usage() <= amount,
            ...
            if registration_only {
                ...
                let refund = amount
                    .checked_sub(Contract::suggested_min_storage_usage())
                    .unwrap();
                if refund > 0 {
            ...
            if registration_only {
                env::panic_str(format!("{}", ERR14_ACC_ALREADY_REGISTERED).as_str());
            } else {
                assert!(
                    locked <= amount + deposited,
                    "Insufficient storage deposit, expected [{}] actually [{}]",
                    locked,
                    amount + deposited
                );
    ...
    fn storage_withdraw(&mut self, amount: Option<U128>) -> StorageBalance {
        ...
        if deposited > 0 {
            ...
            farmer.get_ref_mut().amount = farmer.get_ref_mut().amount.checked_sub(amount).unwrap();
            self.data_mut().farmers.insert(&account_id, &farmer);
            Promise::new(account_id.clone()).transfer(amount);
            self.storage_balance_of(account_id.try_into().unwrap())
                .unwrap()
        } else {
            env::panic_str(format!("Account [{}] not registered", account_id).as_str());
        }
    }
    ...
    fn storage_unregister(&mut self, force: Option<bool>) -> bool {
        ...
        if let Some(farmer) = self.get_farmer_wrapped(&account_id) {
            assert!(
                ...
                "{}",
                STORAGE_UNREGISTER_REWARDS_NOT_EMPTY
            );
            assert!(
                farmer.get_ref().seeds.is_empty(),
                "{}",
                STORAGE_UNREGISTER_SEED_NOT_EMPTY
            );
            self.data_mut().farmers.remove(&account_id);
            self.data_mut().farmer_count = self.data_mut().farmer_count.checked_sub(1).unwrap();
            // TODO: should make sure tranfer is OK with a callback
...
impl Contract {
    ...
    pub(crate) fn assert_storage_usage(&self, account_id: &AccountId) {
        let (locked, deposited) = self.internal_farmer_storage(account_id);
        assert!(deposited > 0, "Account [{}] not registered", account_id);
        assert!(
            locked <= deposited,
            "Insufficient storage deposit, expected [{}] actually [{}]",
            locked,
    ...
    pub(crate) fn internal_register_account(&mut self, account_id: &AccountId, amount: Balance) {
        if let Some(mut farmer) = self.get_farmer_wrapped(&account_id) {
            farmer.get_ref_mut().amount = farmer.get_ref_mut().amount.checked_add(amount).unwrap();
            self.data_mut().farmers.insert(&account_id, &farmer);
        } else {
            self.data_mut().farmers.insert(
                &account_id,
                &VersionedFarmer::new(account_id.clone(), amount),
            );
            self.data_mut().farmer_count = self.data_mut().farmer_count.checked_add(1).unwrap();
        }
    }
}
